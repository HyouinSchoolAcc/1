...
func (a *App) handleTemplate(tpl *template.Template) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		startTime := time.Now()
		log.Printf("[PERF] handleTemplate: START for %s", r.URL.Path)

		dataStart := time.Now()
		data := a.getTemplateData(r)
		log.Printf("[PERF] handleTemplate: getTemplateData took %v", time.Since(dataStart))
		...
		log.Printf("[PERF] handleTemplate: template execution took %v", time.Since(renderStart))
		log.Printf("[PERF] handleTemplate: TOTAL %v for %s", time.Since(startTime), r.URL.Path)
	}
}
...
func (a *App) handleStore(tpl *template.Template) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		sessionData := a.getCurrentUser(r)
		xp := 0
		if sessionData != nil {
			if prof, err := a.profileStore.GetProfile(sessionData.UserID); err == nil {
				xp = prof.XP
			}
		}
		...
		data := map[string]any{
			...
			"CurrentUser": func() string {
				if sessionData != nil { return sessionData.Username }
				return ""
			}(),
			"UserRole": func() string {
				if sessionData != nil { return string(sessionData.Role) }
				return "new_user"
			}(),
			"Language": GetLanguage(r),
			"UserXP":   xp,
			"StoreJSON": template.JS(storeJSON),
		}
		w.Header().Set("Content-Type", "text/html; charset=utf-8")
		if err := tpl.Execute(w, data); err != nil {
			log.Printf("Error rendering store template: %v", err)
			http.Error(w, "Internal Server Error", http.StatusInternalServerError)
		}
	}
}
