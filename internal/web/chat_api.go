...
func (a *App) RegisterChatAPI(r chi.Router) {
	...
	// Admin: clear all chat data
	r.Delete("/api/chat/clear-all", a.handleClearAllChatData)
}
...
// handleClearAllChatData deletes all chat messages, channels, DMs, friends, etc.
func (a *App) handleClearAllChatData(w http.ResponseWriter, r *http.Request) {
	sessionData := a.getCurrentUser(r)
	if sessionData == nil {
		writeJSON(w, http.StatusUnauthorized, errJSON(errors.New("需要登录")))
		return
	...
	if sessionData.Role != RoleEditor {
		writeJSON(w, http.StatusForbidden, errJSON(errors.New("仅限管理员")))
		return
	}

	err := a.chatStore.ClearAllChatData()
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, errJSON(err))
		return
	}
