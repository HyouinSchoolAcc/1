...
func (a *App) RegisterAPI(r chi.Router) {
	...
	r.Post("/api/user-characters/{id}/vote", a.handleToggleCharacterVote)
	...
	// Public: anyone can add comments
	r.Post("/save_comment", a.handleSaveComment)
	...
	// Editor-only: assign personnel_id to characters
	r.Post("/api/assign-personnel", editorAuth(a.handleAssignPersonnel))

	// Editor-only: toggle public visibility for a user's dialogues
	r.Post("/api/toggle-public-user", editorAuth(a.handleTogglePublicUser))
	...
	r.Post("/api/character-defaults/{preset}", editorAuth(a.handleUpdateCharacterDefaults))
	r.Post("/api/sync-character-defaults", editorAuth(a.handleSyncCharacterDefaults))
...
func (a *App) handleGetCurrentUser(w http.ResponseWriter, r *http.Request) {
	...
	impersonatingAs := ""
	if orig := a.authService.sessionManager.IsImpersonating(r); orig != "" {
		impersonatingAs = sessionData.Username
	}
...
func (a *App) handleLoadStructuredWriterFiles(w http.ResponseWriter, r *http.Request) {
	...
	for _, e := range entries {
		...
		// When showing all (no filter), hide legacy items by default
		if categoryFilter == "" && strings.HasPrefix(fileCategory, "legacy") {
			continue
		}
		...
		info := map[string]any{
			...
			"is_public":                   a.isPublicUser(preset, pf.UserID),
			"category":                    fileCategory,
			"rejection_reason":            stringField(data, "rejection_reason", ""),
		}
		for _, k := range []string{"name", "user_name", "real_name", "dialogue_trait", "personnel_id"} {
			if v, ok := data[k]; ok {
				info[k] = v
			}
		}
...
func (a *App) handleSaveWriterFileContent(w http.ResponseWriter, r *http.Request) {
	...
	if !a.checkCharacterOwnership(sessionData, fileData) {
		log.Printf("[SAVE %s] DENIED: User %s not authorized to edit %s", saveID, sessionData.Username, filename)
		writeJSON(w, http.StatusForbidden, errJSON(errors.New("You can only edit your own characters. 你只能编辑自己的角色。")))
		return
	}
	...
	// character_schedule edits are no longer written back to preset files.
	// Schedules are sourced from data/character_profiles.json (https://wl2.studio/descriptions).
...
// handleSaveComment allows anyone (including non-logged-in users) to add a comment
func (a *App) handleSaveComment(w http.ResponseWriter, r *http.Request) {
	body, err := decodeBody(r.Body)
	if err != nil {
		writeJSON(w, http.StatusBadRequest, errJSON(err))
		return
	...
	sentenceRef := stringField(body, "sentence_ref", "")

	if text == "" {
		writeJSON(w, http.StatusBadRequest, errJSON(errors.New("Comment text is required")))
		return
	}
	if author == "" {
		author = "Anonymous"
	...
	comments, _ := fileData["comments"].([]any)
	...
	writeJSON(w, http.StatusOK, map[string]any{"success": true, "message": "Comment saved.", "comment": newComment})
}
...
func (a *App) handleUpdateQCStatus(w http.ResponseWriter, r *http.Request) {
	...
	filename := stringField(body, "filename", "")
	preset := stringField(body, "preset_set", "presets_kurisu")
	newCategory := stringField(body, "category", "pending")
	folder, err := a.getPresetFolder(preset)
	if err != nil || filename == "" {
		writeJSON(w, http.StatusBadRequest, errJSON(errors.New("Filename or preset set not provided")))
		return
	...
	if newCategory != "in_works" && newCategory != "pending" {
		writeJSON(w, http.StatusBadRequest, errJSON(errors.New("Writers can only toggle between in_works and pending")))
		return
	}
	...
	if !a.checkCharacterOwnership(sessionData, data) {
		writeJSON(w, http.StatusForbidden, errJSON(errors.New("You can only edit your own characters. 你只能编辑自己的角色。")))
		return
	}
	...
	writeJSON(w, http.StatusOK, map[string]any{"success": true, "message": "Status updated successfully.", "category": newCategory})
}
...
func (a *App) handleUpdateCharacterTurn(w http.ResponseWriter, r *http.Request) {
	...
	_ = sessionData // ownership check not applicable for in-memory conversation buffers
	body, err := decodeBody(r.Body)
	if err != nil {
		writeJSON(w, http.StatusBadRequest, errJSON(err))
		return
...
func (a *App) handleCreateNewVersion(w http.ResponseWriter, r *http.Request) {
	...
	// Check ownership: writers can only create versions for their own characters
	if !a.checkUserOwnership(sessionData, pf.UserID, preset) {
		writeJSON(w, http.StatusForbidden, errJSON(errors.New("You can only edit your own characters. 你只能编辑自己的角色。")))
		return
	}
	entries, _ := os.ReadDir(folder)
	...
	for _, t := range templateFiles {
		...
		newData := map[string]any{
			...
			"_creation_metadata": map[string]any{
		...
		// Inherit personnel_id from source file
		if pid := stringField(data, "personnel_id", ""); pid != "" {
			newData["personnel_id"] = pid
			newData["personnel_username"] = stringField(data, "personnel_username", "")
		}
...
func (a *App) handleDeleteVersion(w http.ResponseWriter, r *http.Request) {
	...
	// Check ownership: writers can only delete versions for their own characters
	if !a.checkUserOwnership(sessionData, userID, preset) {
		writeJSON(w, http.StatusForbidden, errJSON(errors.New("You can only edit your own characters. 你只能编辑自己的角色。")))
		return
	}
	entries, _ := os.ReadDir(folder)
...
func (a *App) handleAddCharacter(w http.ResponseWriter, r *http.Request) {
	...
	fileData := map[string]any{
		...
		"personnel_id":            sessionData.UserID,
		"personnel_username":      sessionData.Username,
		"values":                  defaults.Values,
		"experiences":             defaults.Experiences,
		"judgements":              defaults.Judgements,
		"abilities":               defaults.Abilities,
...
func (a *App) handleAddTempCharacter(w http.ResponseWriter, r *http.Request) {
	...
	// Get the current user's personnel_id from the session
	personnelID := ""
	if sessionData != nil {
		personnelID = sessionData.UserID
	}
...
func (a *App) handleDeleteCharacter(w http.ResponseWriter, r *http.Request) {
	...
	// Check ownership: writers can only delete their own characters
	if !a.checkUserOwnership(sessionData, userID, preset) {
		writeJSON(w, http.StatusForbidden, errJSON(errors.New("You can only delete your own characters. 你只能删除自己的角色。")))
		return
	}
	entries, _ := os.ReadDir(folder)
	var files []string
...
func (a *App) handleUpdateRealName(w http.ResponseWriter, r *http.Request) {
	...
	// Check ownership: writers can only update their own characters
	if !a.checkUserOwnership(sessionData, userID, preset) {
		writeJSON(w, http.StatusForbidden, errJSON(errors.New("You can only edit your own characters. 你只能编辑自己的角色。")))
		return
	}
...
func (a *App) handleAddDay(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("[ADD_DAY] === Request received ===")
	log.Printf("[ADD_DAY] Raw body keys: %v", func() []string {
	...
	log.Printf("[ADD_DAY] user_id=%q, dup_id=%q, day_number=%d, preset=%q", userID, dupID, dayNumber, preset)
	log.Printf("[ADD_DAY] raw dup_id value from body: %v (type: %T)", body["dup_id"], body["dup_id"])
	...
	// Check ownership: writers can only add days to their own characters
	if !a.checkUserOwnership(sessionData, userID, preset) {
		writeJSON(w, http.StatusForbidden, errJSON(errors.New("You can only edit your own characters. 你只能编辑自己的角色。")))
		return
	}
	...
	// If dup_id was provided, use it; otherwise fall back to first file's dup
	if dupID == "" {
		dupID = existing[0].pf.DupID
		log.Printf("[ADD_DAY] WARNING: No dup_id provided, falling back to first file's dup: %q (file: %s)", dupID, existing[0].name)
	} else {
		log.Printf("[ADD_DAY] Using provided dup_id: %q", dupID)
	}
	log.Printf("[ADD_DAY] Existing files for %s:", userID)
	for i, ex := range existing {
		log.Printf("[ADD_DAY]   [%d] %s (DupID=%q, DayNum=%d)", i, ex.name, ex.pf.DupID, ex.pf.DayNum)
	}

	// Only check conflicts within the same dup
	for _, ex := range existing {
		if ex.pf.DupID == dupID && ex.pf.DayNum == dayNumber {
			writeJSON(w, http.StatusConflict, errJSON(fmt.Errorf("Day %d already exists for %s. Files: %s", dayNumber, userID, ex.name)))
			return
		}
	}

	// Load character info from a file in the same dup
	var firstData map[string]any
	for _, ex := range existing {
		if ex.pf.DupID == dupID {
			_ = a.loadJSON(filepath.Join(folder, ex.name), &firstData)
			break
		}
	}
	if firstData == nil {
		_ = a.loadJSON(filepath.Join(folder, existing[0].name), &firstData)
	}
	characterName := stringField(firstData, "user_name", "Unknown Character")
	aiName := getCurrentCharacterDisplayName(preset)

	// starting intimacy from previous day (within the same dup)
	startIntimacy := 0
	var values, experiences, judgements, abilities interface{}

	for _, ex := range existing {
		if ex.pf.DupID == dupID && ex.pf.DayNum == dayNumber-1 {
	...
	log.Printf("[ADD_DAY] Generated filename: %s", filename)
	log.Printf("[ADD_DAY] Full path: %s", filepath.Join(folder, filename))
	...
	// Inherit personnel_id from existing character files
	personnelID := stringField(firstData, "personnel_id", "")
	personnelUsername := stringField(firstData, "personnel_username", "")
	...
	if personnelID != "" {
		fileData["personnel_id"] = personnelID
		fileData["personnel_username"] = personnelUsername
	}
...
func (a *App) handleDeleteDay(w http.ResponseWriter, r *http.Request) {
	...
	// Check ownership: writers can only delete their own days
	if !a.checkUserOwnership(sessionData, userID, preset) {
		writeJSON(w, http.StatusForbidden, errJSON(errors.New("You can only edit your own characters. 你只能编辑自己的角色。")))
		return
	}
...
func (a *App) handleUpdateSchedule(w http.ResponseWriter, r *http.Request) {
	...
	// Check ownership: writers can only update schedules for their own characters
	if !a.checkCharacterOwnership(sessionData, data) {
		writeJSON(w, http.StatusForbidden, errJSON(errors.New("You can only edit your own characters. 你只能编辑自己的角色。")))
		return
	}
...
func (a *App) handleUpdateDayCategory(w http.ResponseWriter, r *http.Request) {
	// Require at least writer or editor role
	sessionData := a.requireWriterOrEditor(w, r)
	if sessionData == nil {
		return
	}
	...
	if category == "passed" || category == "rejected" {
		if sessionData.Role != RoleEditor {
			writeJSON(w, http.StatusForbidden, errJSON(errors.New("Only editors can approve or reject. 只有编辑才能通过或拒绝。")))
			return
		}
	}
	...
	// For non-editor operations, check ownership
	if category != "passed" && category != "rejected" && !strings.HasPrefix(category, "legacy_") {
		if !a.checkCharacterOwnership(sessionData, data) {
			writeJSON(w, http.StatusForbidden, errJSON(errors.New("You can only edit your own characters. 你只能编辑自己的角色。")))
			return
		}
...
func (a *App) getCharacterScheduleForDay(preset string, dayNumber int) map[string]any {
	...
	case strings.Contains(preset, "kurisu"):
		characterId = "kurisu"
	case strings.Contains(preset, "lin_lu"), strings.Contains(preset, "linlu"):
		characterId = "linlu"
	default:
...
func (a *App) handleUpdateCharacterDefaults(w http.ResponseWriter, r *http.Request) {
	// Only editors can update character defaults
	if sessionData := a.requireEditor(w, r); sessionData == nil {
		return
	}
...
func (a *App) handleSyncCharacterDefaults(w http.ResponseWriter, r *http.Request) {
	// Only editors can sync character defaults
	if sessionData := a.requireEditor(w, r); sessionData == nil {
		return
	}
...
// per character from all approved dialogues (only normal "passed", not "legacy_passed")
func (a *App) handleGetPassedTokens(w http.ResponseWriter, r *http.Request) {
	targetTokensPerChar := 500000

	// Use only the primary (non-duplicate) preset per character to avoid double-counting
	// between EN and CN variants. Both EN/CN presets share the same data folder in practice,
	// but if they are separate folders we only count each character once.
	type presetCharMapping struct {
		preset      string
		characterID string
	}
	presetMappings := []presetCharMapping{
	...
	// Track which files we've already counted (by absolute path) to avoid double-counting
	// if EN and CN presets point to the same folder
	counted := map[string]bool{}

	// Iterate through primary presets only
	for _, mapping := range presetMappings {
		folder, err := a.getPresetFolder(mapping.preset)
		if err != nil {
			continue
		}

		// Also check the _CN variant in case it's a separate folder
		cnPreset := mapping.preset + "_CN"
		cnFolder, _ := a.getPresetFolder(cnPreset)

		folders := []string{folder}
		if cnFolder != "" && cnFolder != folder {
			folders = append(folders, cnFolder)
		}

		for _, f := range folders {
			// Filter hidden users (consistent with handleLoadStructuredWriterFiles)
			// Determine which preset name to use for loading hidden users
			hiddenPreset := mapping.preset
			if f == cnFolder {
				hiddenPreset = cnPreset
			}
			hidden := a.loadHiddenUsers(hiddenPreset)
			...
			for _, e := range entries {
				...
				// Avoid double-counting if EN and CN point to the same folder
				absPath := filepath.Join(f, name)
				if counted[absPath] {
					continue
				}
				counted[absPath] = true

				var data map[string]any
				if err := a.loadJSON(absPath, &data); err != nil {
					continue
				}

				// Only count normal passed dialogues (not legacy_passed)
				category := stringField(data, "category", "pending")
				if category == "passed" {
					dialogue, _ := data["dialogue"].([]any)
					charCount := countDialogueHanChars(dialogue)
					characterTokens[mapping.characterID] += charCount
				}
			}
		}
	}
...
// It reads from stickers_map.json AND auto-discovers new files in sticker subdirectories.
func (a *App) handleGetStickers(w http.ResponseWriter, r *http.Request) {
	stickersDir := filepath.Join(a.cfg.RootDir, "stickers")
	stickersMapPath := filepath.Join(stickersDir, "stickers_map.json")

	var stickersMap map[string]map[string]string
	if err := a.loadJSON(stickersMapPath, &stickersMap); err != nil {
		// If the map file doesn't exist or is invalid, start fresh
		stickersMap = make(map[string]map[string]string)
	}

	// Auto-discover sticker files from subdirectories that aren't in the map yet
	mapDirty := false
	validExts := map[string]bool{".png": true, ".gif": true, ".jpg": true, ".jpeg": true, ".webp": true}

	subdirs, _ := os.ReadDir(stickersDir)
	for _, subdir := range subdirs {
		if !subdir.IsDir() {
			continue
		}
		folderName := subdir.Name()
		folderPath := filepath.Join(stickersDir, folderName)
		...
		for _, f := range files {
			...
			// If this file is not in the map, auto-add it
			if _, exists := stickersMap[f.Name()]; !exists {
				stickersMap[f.Name()] = map[string]string{
					"description": "",
					"folder":      folderName,
				}
				mapDirty = true
	...
	// Persist newly discovered stickers back to the map file
	if mapDirty {
		if mapFile, err := os.Create(stickersMapPath); err == nil {
			encoder := json.NewEncoder(mapFile)
			encoder.SetIndent("", "  ")
			_ = encoder.Encode(stickersMap)
			mapFile.Close()
		}
	}
	...
	for filename, metadata := range stickersMap {
		...
		if folder == "" {
			stickerURL = fmt.Sprintf("/static/stickers/%s", encodedFilename)
		} else {
			stickerURL = fmt.Sprintf("/static/stickers/%s/%s", folder, encodedFilename)
		}
...
// handleAssignPersonnel allows editors to assign a personnel_id to a character (all files for a given user_id).
func (a *App) handleAssignPersonnel(w http.ResponseWriter, r *http.Request) {
	...
	userID := stringField(body, "user_id", "")
	personnelID := stringField(body, "personnel_id", "")
	preset := stringField(body, "preset_set", "presets_kurisu")

	if userID == "" || personnelID == "" {
		writeJSON(w, http.StatusBadRequest, errJSON(errors.New("user_id and personnel_id are required")))
		return
	}
	...
	if updated == 0 {
		writeJSON(w, http.StatusNotFound, errJSON(fmt.Errorf("No files found for user %s in %s", userID, preset)))
		return
	}

	writeJSON(w, http.StatusOK, map[string]any{
		"success":       true,
		"message":       fmt.Sprintf("Personnel ID '%s' assigned to %d files for %s", personnelID, updated, userID),
		"updated_files": updated,
	})
}
...
func (a *App) handleUploadSticker(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := io.Copy(destFile, file); err != nil {
		writeJSON(w, http.StatusInternalServerError, errJSON(fmt.Errorf("failed to write file: %w", err)))
		return
	}
...
func (a *App) handleDebugLog(w http.ResponseWriter, r *http.Request) {
	...
	logEntry := fmt.Sprintf("[%s] [CLIENT %s] [%s] File: %s | Turns: %d | Words: %d",
		timestamp, logType, clientIP, filename, turnCount, wordCount)

	if queuedTurnCount >= 0 {
		logEntry += fmt.Sprintf(" | Queued: %d turns, %d words", queuedTurnCount, queuedWordCount)
	}
...
func (a *App) handleTogglePublicUser(w http.ResponseWriter, r *http.Request) {
	var req struct {
	...
	if req.Preset == "" || req.UserID == "" {
		writeJSON(w, http.StatusBadRequest, map[string]any{"error": "preset and user_id required"})
		return
	}
	if _, err := a.getPresetFolder(req.Preset); err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]any{"error": "unknown preset"})
		return
	}
	newVal, err := a.togglePublicUser(req.Preset, req.UserID)
	if err != nil {
		log.Printf("Failed to toggle public user %s in %s: %v", req.UserID, req.Preset, err)
		writeJSON(w, http.StatusInternalServerError, map[string]any{"error": "failed to save"})
		return
	}
	writeJSON(w, http.StatusOK, map[string]any{"success": true, "is_public": newVal})
}
