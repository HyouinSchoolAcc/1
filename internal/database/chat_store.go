...
func (cs *ChatStore) GetMessages(channelID string, limit int, before string) []*ChatMessage {
	...
	for rows.Next() {
		...
		msg.Reactions = cs.getMessageReactions(msg.ID)
...
// ClearAllChatData deletes all messages, reactions, DMs, conversations,
// non-default channels, friends, and presence data.
func (cs *ChatStore) ClearAllChatData() error {
	tx, err := cs.db.BeginTx()
	if err != nil {
		return err
	}
	...
	tables := []string{
		...
		"DELETE FROM chat_channels WHERE is_default = 0",
		"DELETE FROM friends",
	...
	// Reset last_message_at on default channel
	if _, err := tx.Exec("UPDATE chat_channels SET last_message_at = NULL"); err != nil {
		return err
	}
