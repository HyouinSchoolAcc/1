...
-- Composite indexes for batch reaction queries (fixes N+1 pattern)
CREATE INDEX IF NOT EXISTS idx_lounge_reactions_post_emoji ON lounge_reactions(post_id, emoji);
CREATE INDEX IF NOT EXISTS idx_lounge_reactions_reply_emoji ON lounge_reactions(reply_id, emoji);
...
-- Composite index for batch message reaction queries (fixes N+1 pattern)
CREATE INDEX IF NOT EXISTS idx_chat_reactions_message_emoji ON chat_message_reactions(message_id, emoji);
