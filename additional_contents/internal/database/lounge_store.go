...
// GetPosts retrieves posts, sorted by timestamp (newest first), with optional limit
func (ls *LoungeStore) GetPosts(limit ...int) []*LoungePost {
	// Default limit to 100 if not specified, use -1 for no limit
	queryLimit := 100
	if len(limit) > 0 && limit[0] > 0 {
		queryLimit = limit[0]
	} else if len(limit) > 0 && limit[0] == -1 {
		queryLimit = -1 // No limit
	}

	query := `SELECT id, type, author_character, author_user_id, author_username, content, 
		 timestamp, reply_count, is_prompt, COALESCE(prompt_date, '') 
	...
	for rows.Next() {
		...
		post.Timestamp = parseTimestamp(timestampStr)
		post.Type = PostType(typeStr)
		post.Reactions = make(map[string]int) // Initialize empty, will be filled by batch query

		posts = append(posts, post)
		postIDs = append(postIDs, post.ID)
	}

	// Batch load all reactions for all posts in a single query (fixes N+1)
	if len(postIDs) > 0 {
		reactionsMap := ls.batchGetPostReactions(postIDs)
		for _, post := range posts {
			if reactions, ok := reactionsMap[post.ID]; ok {
...
// GetReplies retrieves all replies for a post with optional limit
func (ls *LoungeStore) GetReplies(postID string, limit ...int) []*LoungeReply {
	query := `SELECT id, post_id, COALESCE(parent_reply_id, ''), author_character, 
		 author_user_id, author_username, content, timestamp 
	...
	rows, err := ls.db.Query(query, postID)
	if err != nil {
		return []*LoungeReply{}
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		reply.Reactions = make(map[string]int) // Initialize empty, will be filled by batch query
		replies = append(replies, reply)
		replyIDs = append(replyIDs, reply.ID)
	}

	// Batch load all reactions for all replies in a single query (fixes N+1)
	if len(replyIDs) > 0 {
		reactionsMap := ls.batchGetReplyReactions(replyIDs)
		for _, reply := range replies {
			if reactions, ok := reactionsMap[reply.ID]; ok {
...
// batchGetPostReactions loads reactions for multiple posts in a single query
// Returns a map of postID -> map[emoji]count
func (ls *LoungeStore) batchGetPostReactions(postIDs []string) map[string]map[string]int {
	...
	query := fmt.Sprintf(
		"SELECT post_id, emoji, COUNT(*) FROM lounge_reactions WHERE post_id IN (%s) GROUP BY post_id, emoji",
	...
	for rows.Next() {
		var postID, emoji string
		var count int
		if err := rows.Scan(&postID, &emoji, &count); err == nil {
			if result[postID] == nil {
				result[postID] = make(map[string]int)
			}
			result[postID][emoji] = count
		}
	}
...
// batchGetReplyReactions loads reactions for multiple replies in a single query
// Returns a map of replyID -> map[emoji]count
func (ls *LoungeStore) batchGetReplyReactions(replyIDs []string) map[string]map[string]int {
	result := make(map[string]map[string]int)
	if len(replyIDs) == 0 {
		return result
	}
	...
	// Build placeholder string for IN clause
	placeholders := make([]string, len(replyIDs))
	...
	query := fmt.Sprintf(
		"SELECT reply_id, emoji, COUNT(*) FROM lounge_reactions WHERE reply_id IN (%s) GROUP BY reply_id, emoji",
		strings.Join(placeholders, ","),
	)
	...
	for rows.Next() {
		var replyID, emoji string
		var count int
		if err := rows.Scan(&replyID, &emoji, &count); err == nil {
			if result[replyID] == nil {
				result[replyID] = make(map[string]int)
			}
			result[replyID][emoji] = count
		}
	}
